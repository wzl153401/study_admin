    ...
    <PageContainer>
      <Card className={styles.warp}>
        <Form className={styles.form} form={form1} onFinish={onFinish} >
          <Row>
            <Col span={6} >
              <FormItem labelCol={{ span: 6 }} name="phone" wrapperCol={{ span: 10 }} label="用户手机号">
                <Input
                  placeholder="请输入"
                />
              </FormItem>
            ...
            <Col span={6}>
              <FormItem labelCol={{ span: 6 }} wrapperCol={{ span: 10 }} initialValue={0} name='is' label="支付类型">
                <Select
                  placeholder=""
                >
                  {[{ id: 0, name: '全部' }, { id: 1, name: '未支付' }, { id: 2, name: '已支付' }].map((item: any) => {
            ...
            <Col span={6}>
              <FormItem labelCol={{ span: 6 }} wrapperCol={{ span: 10 }} initialValue={0} name='is' label="订单类型">
                <Select
                  placeholder=""
                >
                  {[{ id: 0, name: '全部' }, { id: 1, name: '正常订单' }, { id: 2, name: '赠送订单' }].map((item: any) => {
                    return (<Select.Option value={item.id} key={item.id}>{item.name}</Select.Option>)
                  })}
                </Select>
